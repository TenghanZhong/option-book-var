...
def validate_one(results_dir: Path) -> dict:
    audit_path = results_dir / "book_var_feature_columns_no_leakage.csv"
    if not audit_path.exists():
        return {"results_dir": str(results_dir), "status": "missing_audit", "forbidden_count": None}
    ...
    if "is_realized_current_marking_diagnostic" in audit.columns:
        ...
        return {"results_dir": str(results_dir), "status": "unreadable_audit_schema", "forbidden_count": None}
    return {
...
def main() -> None:
    ...
    rows = [validate_one(Path(p)) for p in result_dirs]
